...
class BudayaCollection(object):
    ...
    def exportToCSV(self, fileName):
        try:
            csv_open = open(fileName, 'w')
            csv_write = csv.writer(csv_open, delimiter=",")
            for dic in self.koleksi:
                csv_write.writerow(self.koleksi[dic])
            csv_open.close()
    ...
    def tambah(self, aName, aTipe, aProv, anURL):
        ...
        if aName not in self.koleksi:
            newBudayaItem = BudayaItem(
                aName.strip(), aTipe.strip(), aProv.strip(), anURL.strip())
